...
def load_data():
    ...
    try:
        ...
        st.error("Data files not found. Please ensure model_predictions.csv and model_comparison_results.csv are in the directory.")
        return None, None

@st.cache_data
def load_state_coordinates():
    """Load US state coordinates for mapping"""
    # Sample state coordinates (you can expand this)
    state_coords = {
...
def main():
    ...
    # Sidebar
    with st.sidebar:
        st.image("https://via.placeholder.com/300x100/1f77b4/ffffff?text=ML+Traffic+Forecasting", use_column_width=True)
    ...
    with col1:
        ...
        st.write(f"**{best_model}** achieves the highest R² score of {comparison['R²'].max():.3f}")
